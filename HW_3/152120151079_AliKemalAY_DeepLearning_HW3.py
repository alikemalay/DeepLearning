...
def trainPerceptron(inputs, t, weights, rho, iterNo):
    for iter in range(iterNo):
        for i in range(inputs.shape[0]):
            X=inputs[i]
            sumA=np.sum(np.dot(X,weights))
            y = sigmoid_activation(sumA,False)
            deltaW = rho*(t[i]-y)*sigmoid_activation(y,True) * X
...
def testPerceptron(sample_test, weights):
    y = 0
    y = np.sum(np.dot(sample_test,weights))
    y = sigmoid_activation(y,False);
...
# Print Expected/Predicted values #
